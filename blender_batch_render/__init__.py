...
def render(input_path, output_path, template_path='template.blend', target_texture='target-texture', target_scene='Scene', is_animation=False, blender_path='blender'):
    path_to_blender_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'blenderScript.py')
    ...
    cmd = [blender_path, format_path(template_path), '--background', '--python', format_path(path_to_blender_script), '--']
